...
const DATABASE_URL = 'postgresql://neondb_owner:npg_5LKtIPu1SAOz@ep-spring-brook-ang1zx8s-pooler.c-6.us-east-1.aws.neon.tech/neondb?sslmode=require';
...
const USERS = [
  ...
  { user_id:"System", name:"System",                   email:"system@ardcity.com",   password:"system", role:"Team", dept:"Executive", status:"Active", joined:"2020-01-01", phone:null },
];
...
async function seed() {
  ...
  const existingUsers = await sql`SELECT count(*)::int as cnt FROM users`;
  if (existingUsers[0].cnt > 0) {
    console.log('⚠️  Database already has data. Clearing tables first...');
    await sql`DELETE FROM audit_logs`;
    await sql`DELETE FROM notifications`;
    await sql`DELETE FROM comments`;
    await sql`DELETE FROM attachments`;
    await sql`DELETE FROM tasks`;
    await sql`DELETE FROM directives`;
  ...
  for (const u of USERS) {
    await sql`INSERT INTO users (user_id, name, email, password, role, dept, status, joined, phone)
      VALUES (${u.user_id}, ${u.name}, ${u.email}, ${u.password}, ${u.role}, ${u.dept}, ${u.status}, ${u.joined}, ${u.phone})`;
  }
  console.log(`   ✓ ${USERS.length} users inserted`);

  // 2. Directives + nested tasks/comments/attachments
  ...
  for (const d of DIRECTIVES) {
    ...
    for (const t of d.tasks) {
      await sql`INSERT INTO tasks (task_id, directive_id, title, status, assigned_to, due, progress)
        VALUES (${t.task_id}, ${d.directive_id}, ${t.title}, ${t.status}, ${t.assigned_to}, ${t.due}, ${t.progress})`;
    }

    for (const c of d.comments) {
      await sql`INSERT INTO comments (comment_id, directive_id, user_id, text, created_at)
        VALUES (${c.comment_id}, ${d.directive_id}, ${c.user_id}, ${c.text}, ${c.ts})`;
    }

    for (const a of d.attachments) {
      await sql`INSERT INTO attachments (directive_id, name, size, uploaded)
        VALUES (${d.directive_id}, ${a.name}, ${a.size}, ${a.uploaded})`;
    }
  }
  console.log(`   ✓ ${DIRECTIVES.length} directives inserted (with tasks, comments, attachments)`);
  ...
  console.log(`   ✓ ${AUDIT.length} audit logs inserted`);
